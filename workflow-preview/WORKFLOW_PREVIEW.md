...
jobs:
  build-and-publish:
    ...
    steps:
      ...
      # 2) Node con cache SOLO se esiste package-lock.json
      ...
      # 3) Install deps (usa ci se c'è il lock, altrimenti install)
      - name: Install deps
        working-directory: source
        run: |
      ...
      # 4) Patch ai typo comuni di AI Studio
      ...
      # 5) Build Vite per GitHub Pages (base /gestore/)
      ...
      # 6) Patch PWA (manifest/icona, fallback SPA)
      ...
      # 6.1) SW controllato + pwa-update (banner solo su versione nuova)
      ...
      # 6.2) Pagina /reset: VALIDATE + CONSUME, poi redirect all’app
      ...
      # 7) Deploy su gh-pages della repo "gestore"
